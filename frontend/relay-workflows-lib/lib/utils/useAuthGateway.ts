...
export function useAuthGateway(): boolean {
  ...
  // The ensures that the value can be reliably set in the container at runtime
  // via configure.sh
  ...
  if (
    ...
    runtimeValue !== "" &&
    !runtimeValue.includes("{{") // during local development, it still has it's placeholder value
  ) {
