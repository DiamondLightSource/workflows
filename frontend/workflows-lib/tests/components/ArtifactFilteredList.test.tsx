...
describe("ArtifactFilteredList", () => {
  it("renders all artifacts by default - ascending order by artefact name", () => {
    render(<ArtifactFilteredList artifactList={mockArtifacts} />);
    const artifactElements = screen.getAllByRole('row').slice(1);
    const artifactNames = artifactElements.map(el => (el as HTMLTableRowElement).cells[0].textContent);
    expect(artifactNames).toEqual(['image1.png', 'image2.png', 'main.log', 'textfile.txt'])
  });
  ...
  it('sorts artifact by name in descending order', () => {
    ...
    const artifactElements = screen.getAllByRole('row').slice(1);
    const artifactNames = artifactElements.map(el => (el as HTMLTableRowElement).cells[0].textContent);
    expect(artifactNames).toEqual(['textfile.txt', 'main.log', 'image2.png', 'image1.png']);
  });
  ...
  it('sorts artifact by parent-task in ascending order', () => {
    ...
    const artifactElements = screen.getAllByRole('row').slice(1);
    const artifactNames = artifactElements.map(el => (el as HTMLTableRowElement).cells[1].textContent);
