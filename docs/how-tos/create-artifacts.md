...
This guide will describe how to create artifacts and export them at the end of a workflow run. These artifacts are then made available on the dashboard where they can be downloaded.

## The Workflow

We start with a basic workflow - this is taken from the [writeable containers example](/how-tos/create-writeable-container) example. This is a basic workflow that uses a busybox image to execute some arbitrary bash command - where the bash is defined by a parameter - `command`.
This workflow also has a local-volume mouted to it to store some files in.
...
We must modify the workflow template to expect to create an artifact. Add the `output` tag with the name and path of the expected artifact.
