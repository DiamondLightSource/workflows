      ...
      if retryBehaviour and retryBehaviour.alertSlackChannelId:
        ...
        dlqTrigger = {"template": {
            ...
            "slack": {
              ...
              "message":
                (f":rotating_light: *Trigger {name} has failed to submit a workflow from template {template}*.\n"
                 "Please check that the workflow template name and parameters are correct in the trigger template. "
                 "Assistance and downtime information is available from the <#C08NYJSGMFD> channel") # links to the workflows channel
            }
          },
          "atLeastOnce": True,
        }
