...
pub fn lint(args: LintArgs) {
    ...
    let mut manifest_errors = match config["manifests"].as_vec() {
        Some(manifest_paths) => {
            ...
            let manifest_names = manifest_paths.iter().map(|yaml| yaml.as_str().unwrap()).collect();
    ...
    let helm_errors = match config["helmManifests"].as_vec() {
        Some(manifest_paths) => {
            ...
            helm_manifests(&args.base_path, manifest_paths)
        }
        None => {
            println!(
                "{}",
...
fn helm_manifests(base_path: &Path, manifests: &[Yaml]) -> Vec<String> {
    println!("----------------------------------------------");
    println!("{}", "[INFO] Beginning linting for helm manifests...".blue());
    ...
    for manifest_path in &abs_manifest_paths {
        println!(
            "{} {}",
            "[INFO] Running `helm template` on:".blue(),
            manifest_path.display()
        );
        ...
        let output_manifests: Vec<String> = split_manifests.iter().enumerate().map(|(i, template_content)| {
        ...
        return conventional_manifests(Path::new("/"), borrowed_manifest);
    }
    vec![]
}