...
mod tests {
    ...
    async fn single_workflow_query() {
        let workflow_name = "numpy-benchmark-r9csr";
        ...
        response_file_path.push("get-single-workflow.json");
        let argo_response = std::fs::read_to_string(response_file_path).unwrap();
        let path = format!(
            "/api/v1/workflows/{}{}-{}/{}",
            proposal_code, proposal_number, number, workflow_name
        );
        ...
        let query = r#"
            query {
                workflow(name: "numpy-benchmark-r9csr", visit: {proposalCode: "mg", proposalNumber: 36964, number: 1}) {
                    name
                }
            }
        "#;
