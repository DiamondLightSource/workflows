...
use openidconnect::{AuthorizationCode, ClientId, ClientSecret, IssuerUrl, reqwest};
...
pub async fn callback(State(state): State<AppState>, Query(params): Query<CallbackQuery>) -> Result<()> {
    ...
    let session = state.session.get(&params.state).await.ok_or("Invalid session");
