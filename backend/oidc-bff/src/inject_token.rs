...
pub async fn inject_token_from_session(
    ...
    inject_token(state, session, req, next).await
}

trait TokenStore {
    async fn get(&self) -> Result<Option<TokenSessionData>>;
    async fn insert(&self, token: TokenSessionData) -> Result<()>;
}

impl TokenStore for Session {
    async fn get(&self) -> Result<Option<TokenSessionData>> {
        Ok(self.get(TokenSessionData::SESSION_KEY).await?)
    }
...
// Read token from session
    let token: Option<TokenSessionData> = store.get().await?;
    if let Some(mut token) = token {
        if (token.access_token_is_expired()) {
            token = refresh_token_and_update_session(&state, &token, &store).await?;
        }
