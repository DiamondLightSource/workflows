...
struct ServeArgs {
    ...
    #[arg(
        long,
        env = "OIDC_ISSUER_URL",
        default_value = "https://identity.diamond.ac.uk/realms/dls"
    )]
    oidc_issuer_url: Uri,
    /// Client ID used to connect to OIDC issuer (only required for token introspection)
    #[arg(long, env = "OIDC_CLIENT_ID", default_value = "")]
    oidc_client_id: String,
    /// Client secret user to connect to OIDC issuer (only required for token introspection)
    #[arg(long, env = "OIDC_CLIENT_SECRET", default_value = "")]
    oidc_client_secret: Option<String>,
    /// Accepted audiences
    #[arg(long, env = "OIDC_AUDIENCES", value_delimiter = ',', default_value = "workflows-cluster,graph", num_args = 1..)]
