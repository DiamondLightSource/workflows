...
impl WorkflowTemplate {
    ...
    async fn template_url(&self) -> anyhow::Result<&String> {
        ...
        // let gvk = GroupVersionKind::gvk("argoproj.io/v1alpha", "v1", "Application");
        let gvk = GroupVersionKind::gvk("argoproj.io", "v1alpha", "Application");
        ...
        // let app = api.get("example-manifests-group").await?;
        // let labels = app.metadata.labels.clone().unwrap_or_default();
        // println!("Object: {}", app.name_any());
        // println!("Labels: {:?}", labels);
