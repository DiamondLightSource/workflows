...
impl ValidatedAuthToken {
    pub async fn from_typed_header(
        ...
        return match authorization_header {
            Some(bearer_token) => validate_token(bearer_token.0)
                .await
                .unwrap_or_else(|it| ValidatedAuthToken::Failed(it.to_string())),
...
async fn validate_token(token: Authorization<Bearer>) -> anyhow::Result<ValidatedAuthToken> {
    ...
    let provider_metadata = CoreProviderMetadata::discover_async(
        IssuerUrl::new(
            "https://authn.diamond.ac.uk/realms/master/protocol/openid-connect/token".to_string(),
        )?,
        &http_client,
    )
    .await?;
    ...
    let client = client.set_introspection_url(IntrospectionUrl::new(
        "https://authn.diamond.ac.uk/realms/master/protocol/openid-connect/token/introspect"
            .to_string(),
    )?);
